$(document).ready(function() {
	////////////////////////////////////////////////////////////////////////////////////
	//SIDEBAR IMAGE RANDOMIZE
	
	var sideran = ['./images/sidebar.jpg','./images/sidebar-2.jpg','./images/sidebar-3.jpg','./images/sidebar.jpg','./images/sidebar-2.jpg','./images/sidebar-3.jpg','./images/sidebar.jpg','./images/sidebar-2.jpg','./images/sidebar-3.jpg','./images/sidebar.jpg','./images/sidebar-2.jpg','./images/sidebar-3.jpg'];
	var randomIndex = Math.floor(Math.random() * sideran.length);
	$('#sidebar-img').attr('src',sideran[randomIndex]);
	
	
	var sideranind = ['http://aidevserver.co/projects/casels/public/images/sidebar.jpg','http://aidevserver.co/projects/casels/public/images/sidebar-2.jpg','http://aidevserver.co/projects/casels/public/images/sidebar-3.jpg','http://aidevserver.co/projects/casels/public/images/sidebar.jpg','http://aidevserver.co/projects/casels/public/images/sidebar-2.jpg','http://aidevserver.co/projects/casels/public/images/sidebar-3.jpg','http://aidevserver.co/projects/casels/public/images/sidebar.jpg','http://aidevserver.co/projects/casels/public/images/sidebar-2.jpg','http://aidevserver.co/projects/casels/public/images/sidebar-3.jpg'];
	var randomIndexind = Math.floor(Math.random() * sideranind.length);
	$('#sidebar-img-ind').attr('src',sideranind[randomIndex]);
	...
	$('#left-fixed-drop').on('click', function() {
		var hours = $('#hours-check-').css('display');
		...
		if (hours === 'none') {
			...
			if (!$('#sub-nav-expandable').is('.down')) {
				$('#sub-nav-expandable').css('paddingTop', '35px').slideDown();
				$('#sub-nav-expandable').addClass('down');
				$('#left-fixed-drop i').addClass('fa-times').removeClass('fa-bars');
				$('#left-fixed-drop').css('background-color', 'rgb(44, 126, 213)').css('color', 'rgb(143, 0, 5)');
			} else {
				$('#sub-nav-expandable').css('paddingTop', '10px').slideUp();
				$('#sub-nav-expandable').removeClass('down');
				$('#left-fixed-drop i').addClass('fa-bars').removeClass('fa-times');
				$('#left-fixed-drop').css('background-color', 'inherit').css('color', 'white');
			}
	...
	var featuredheader = ['http://aidevserver.co/projects/casels/public/images/atm-featured.jpg','http://aidevserver.co/projects/casels/public/images/parking-featured.jpg','http://aidevserver.co/projects/casels/public/images/summer-banner.jpg'],
			i = 0,
        $body = $('#header-news-annoucement');
    
    	setInterval(function(){ 
			$body.attr('src', featuredheader[i++ % featuredheader.length]);
		  	}, 5000);
...
//Carousel start rotating
	...
	////////////////////////////////////////////////////////////////////////////////////
	//ABOUT SLIDER RESIZING	

	var aw = 1000;
	var ah = 667;
	var sw = $('.about-page-slider').width();
	var oh = ah / aw * sw;
	...
	$(window).load(function() {
		$('.about-page-slider').css('height', oh);
	});

	$(window).resize(function() {
		if ($('.about-page-slider').width() > sw) {
			sw = $('.about-page-slider').width();
		} else if ($('.about-page-slider').width() < sw) {
			sw = $('.about-page-slider').width();
		}

		$('.about-page-slider').css('height', sh);
	...
	$(document).on('click', '.cl-next-1', function() {
		...
		if (title === '') {
			...
			alert('Please Select if you also want to upload this product to eBay');
			$('#listingType').focus();
		} else if (listingType == 'Yes') {
			//hide part one for ebay
		...
		} else if (listingType == 'No') {
			//hide part one for just store
			$('#create-part-one').hide();
			//show just store part
			$('#create-part-two-store').show();
			//button functionality
			$('.cl-next-1').addClass('cl-next-2').removeClass('cl-next-1');
			$('.cl-previous-1').addClass('cl-previous-2').removeClass('cl-previous-1');
	...
	$(document).on('click', '.cl-next-2', function() {
		...
		var conditionstore = $('#item-condition-store').val();
		var listingType = $('#on_ebay').val();
		
		if (category === '' && listingType =='Yes') {
			alert('Please Insert a Category');
			$('#item-category').focus();
		} else if (duration === 'Select' && listingType =='Yes') {
			alert('Please Insert a Valid Duration');
			$('#item-duration').focus();
		} else if (condition === 'Select' && listingType =='Yes') {
			alert('Please Insert a Valid Condition');
			$('#item-condition').focus();
		}else if (conditionstore === 'Select') {
			alert('Please Insert a Valid Condition');
			$('#item-condition').focus();
		}else if (listingType == 'No'){
			$('#create-part-two').hide();
			$('#create-part-two-store').hide();
			$('#create-part-three-store').show();
			$('#create-submit').show();
	...
	$(document).on('click', '.cl-previous-3', function() {
		...
		if (listingType == 'Yes') {
			...
			//hide part three for just store
			$('#create-part-three').hide();
			//show just store part
			$('#create-part-two-store').show();
			//button functionality
